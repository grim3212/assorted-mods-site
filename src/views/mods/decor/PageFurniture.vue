<template>
  <div id="content">
    <h1><a name="furniture">Furniture</a></h1>
    <p>
      Furniture are a way to spruce up your builds with some unique blocks. Every furniture block
      should support being <i>waterlogged</i> as well as being placed on all sides of a block.
    </p>
    <h2><a name="table">Colorizer Table</a></h2>
    <p>
      Tables are part of some of the new furniture. Tables when placed to one another automatically
      place the legs at the correct positions for a table so you can have enough room to eat your
      cereal. Tables will adjust the legs no matter what side of a block the table was placed on so
      even upside down and on the sides it will look like the table was just flipped or on its side.
    </p>
    <img src="\src\assets\mods\decor\table_recipe.png" />
    <h2><a name="chair">Colorizer Chair</a></h2>
    <p>Chairs go with your tables. These chairs re-orient depending on which way you place them.</p>
    <img src="\src\assets\mods\decor\chair_recipe.png" />
    <img src="\src\assets\mods\decor\chairs_tables.png" />
    <h2><a name="stool">Colorizer Stool</a></h2>
    <p>
      Stools are smaller than chairs and go quite well when a <router-link class="text-blue-300" :to="{ name: 'Planter' }">Planter Pot</router-link> is placed on top of them.
    </p>
    <img src="\src\assets\mods\decor\stool_recipe.png" />
    <img src="\src\assets\mods\decor\stool_pot.png" />
    <h2><a name="counter">Colorizer Counter</a></h2>
    <p>Counters are like the tables except without the legs so they just kinda float there.</p>
    <img src="\src\assets\mods\decor\counter_recipe.png" />
    <h2><a name="lamppost">Colorizer Lamp Post</a></h2>
    <p>
      Lamp Posts are 3 blocks tall when placed and emit light from the top block. If you break any
      of the 3 blocks they all break. They are a good way to make nice looking lighting.
    </p>
    <img src="\src\assets\mods\decor\lamp_post_recipe.png" />
    <img src="\src\assets\mods\decor\lamp_posts.png" />
    <h1><a name="vanilla">Vanilla Alternatives</a></h1>
    <p>
      The following blocks are Colorizer alternatives to vanilla blocks. They work just like the
      vanilla counterparts but can be colorized just like any of the other colorizer blocks.
    </p>
    <h2><a name="door">Colorizer Door</a></h2>
    <img src="\src\assets\mods\decor\door_recipe.png" />
    <h2><a name="trapdoor">Colorizer Trap Door</a></h2>
    <img src="\src\assets\mods\decor\trapdoor_recipe.png" />
    <h2><a name="fence">Colorizer Fence</a></h2>
    <img src="\src\assets\mods\decor\fence_recipe.png" />
    <h2><a name="fencegate">Colorizer Fence Gate</a></h2>
    <img src="\src\assets\mods\decor\fencegate_recipe.png" />
    <h2><a name="wall">Colorizer Wall</a></h2>
    <img src="\src\assets\mods\decor\wall_recipe.png" />
    <h2><a name="stairs">Colorizer Stairs</a></h2>
    <img src="\src\assets\mods\decor\stairs_recipe.png" />
    <h2><a name="slab">Colorizer Slab</a></h2>
    <img src="\src\assets\mods\decor\slab_recipe.png" />
  </div>
</template>
